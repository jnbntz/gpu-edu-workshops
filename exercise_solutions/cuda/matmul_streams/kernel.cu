...
/* copy tile of C to device, each stream does a different tile of C */
      ...
      for( int i = 0; i < localStreams; i++ )
      {
        int cOffset = INDX( rowTile[i], colTile[i], linearTiles ) *
                            tileSize * tileSize;
        checkCUDA( cudaMemcpyAsync( d_c[i], &p_c[cOffset], tileBytes,
...
/* stream A into device */
        ...
        int aOffset = INDX( rowTile[i], k, linearTiles ) *
                            tileSize * tileSize;
        checkCUDA( cudaMemcpyAsync( d_a[i], &p_a[aOffset], tileBytes,
                     cudaMemcpyHostToDevice, stream[i] ) );
        } /* end for */
...
/* stream B into device */
        ...
        int bOffset = INDX( k, colTile[i], linearTiles ) *
                            tileSize * tileSize;
        checkCUDA( cudaMemcpyAsync( d_b[i], &p_b[bOffset], tileBytes,
                     cudaMemcpyHostToDevice, stream[i] ) );
...
/* call the cublas gemm function */
          ...
          checkCUBLAS( cublasSgemm( handle, CUBLAS_OP_N, CUBLAS_OP_N,
                 ...
                 (float *)&alpha,
                 (float *)d_a[i], tileSize,
                 (float *)d_b[i], tileSize,
                 (float *)&beta,
                 (float *)d_c[i], tileSize ) );
          }
          else
          {
          checkCUBLAS( cublasDgemm( handle, CUBLAS_OP_N, CUBLAS_OP_N,
                 tileSize, tileSize, tileSize,
                 (double *)&alpha,
                 (double *)d_a[i], tileSize,
                 (double *)d_b[i], tileSize,
                 (double *)&beta,
                 (double *)d_c[i], tileSize ) );
          }
...
/* copy tile of C back to host */
      ...
      for( int i = 0; i < localStreams; i++ )
      {
        int cOffset = INDX( rowTile[i], colTile[i], linearTiles ) *
                            tileSize * tileSize;
        checkCUDA( cudaMemcpyAsync( &p_c[cOffset], d_c[i], tileBytes,
