...
def train(batch_size, hidden_layer, num_epochs, activation):
    ...
    conv1 = mx.sym.Convolution(data=data, kernel=(5,5), stride=(1,1),
            num_filter=16, name='conv1')

    act1 = mx.sym.Activation(data=conv1, name='act1', act_type=activation)

    pool1 = mx.sym.Pooling(data=act1, name='pool1', kernel=(2,2), 
            stride=(2,2), pool_type='max' )

    conv2 = mx.sym.Convolution(data=data, kernel=(5,5), stride=(1,1),
            num_filter=24, name='conv2')

    act2 = mx.sym.Activation(data=conv2, name='act2', act_type=activation)

    pool2 = mx.sym.Pooling(data=act2, name='pool2', kernel=(2,2), 
            stride=(2,2), pool_type='max' )

    fc1 = mx.sym.FullyConnected(data=pool2, name='fc1', num_hidden=hidden_layer)

    act1 = mx.sym.Activation(data=fc1, name='act1', act_type=activation)
    ...
    lr_sched = mx.lr_scheduler.FactorScheduler(10000, 0.75)

    model.fit(
        train_data=train_iter,
        eval_data=val_iter,
        num_epoch=num_epochs,
        optimizer_params={'learning_rate':0.1, 'lr_scheduler':lr_sched},
